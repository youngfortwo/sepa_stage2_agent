#!/usr/bin/env python3
"""超跌反弹选股独立定时扫描任务（与 SEPA Stage2 部署在同一台扫描机上）。

触发模型与 sepa_stage2_job.py 完全一致（launchd 只是"哑触发器"：开机 + 每 5 分钟
唤起一次本脚本），执行时机由 agent_config.json 控制：
    run_time           每日执行时间（与 SEPA Stage2 相同，默认 18:00）
    boot_run           开机是否立即执行（当天未执行时）
    boot_force         开机是否强制执行（忽略当天已执行标记）
    check_trading_day  是否跳过非交易日
    enabled            总开关

完整流程：
1. 交易日判断
2. 分批调用 oversold_rebound_scanner.py（默认 200 只/批，单批 900s 超时）
3. 合并去重结果写入本地 SQLite（oversold.db 表 oversold_candidates）
4. HTTP POST 上报主机 stock_server（/api/oversold/upload），失败重试 3 次

参数速查：
    --force            手动试跑（忽略所有检查，不写"已执行"标记）
    --boot-force       手动正式重跑（完成后更新标记）
    --total/--batch    扫描总数 / 每批数量（默认 5000 / 200）
    --no-upload        只落本地 SQLite，不上报
    --reupload DATE    跳过扫描，从本地 SQLite 补传指定日期数据
"""
...
SCANNER_SCRIPT = "oversold_rebound_scanner.py"
BATCH_PREFIX = "job_ob_"
TABLE = "oversold_candidates"
DB_DEFAULT = "oversold.db"
UPLOAD_PATH = "/api/oversold/upload"


class _TimeStampedWriter:
    """stdout/stderr 包装：每行自动加时间前缀（launchd 日志无时间戳，包装后补齐）。"""
...
if os.environ.get("_OB_JOB_TS") != "1":  # 防重复包装
    os.environ["_OB_JOB_TS"] = "1"
...
def _prevent_sleep_during_scan() -> None:
    """扫描期间阻止 macOS 闲置睡眠（caffeinate -w 绑定本进程 PID，进程退出自动释放）。"""
...
MARKER_FILE = Path(__file__).parent / "oversold_last_run_marker.txt"   # 当天已执行标记
LOCK_FILE = Path(__file__).parent / ".oversold_job.lock"               # 单实例锁
...
def run_batches(total: int, batch: int) -> pd.DataFrame:
    """分批调用 oversold_rebound_scanner.py，增量合并 batch_results/job_ob_*.csv。"""
    batch_dir = Path("batch_results")
    batch_dir.mkdir(exist_ok=True)
    for f in batch_dir.glob(f"{BATCH_PREFIX}*.csv"):
        try:
            f.unlink()
    ...
    for batch_no in range(total_batches):
        ...
        try:
            proc = subprocess.run(
                [sys.executable, SCANNER_SCRIPT,
                 ...
                 "--output", f"batch_results/{BATCH_PREFIX}{offset}.csv",
                 "--sleep-seconds", "0.15"],
        ...
        part = f"batch_results/{BATCH_PREFIX}{offset}.csv"
        try:
...
def _load_agent_config() -> dict:
    """读取同目录 agent_config.json（deploy.sh 生成）：run_time 等执行时机与 SEPA Stage2 共用。"""
    ...
    try:
        ...
        print(f"WARN agent_config.json 解析失败（{e}），回退默认配置 run_time=18:00: {p}", file=sys.stderr)
        return {}


def parse_args() -> argparse.Namespace:
    cfg = _load_agent_config()
    parser = argparse.ArgumentParser(description="超跌反弹选股 standalone daily job.")
    parser.add_argument("--server", default=cfg.get("server", ""), help="主机 stock_server 地址（默认读 agent_config.json）")
    ...
    parser.add_argument("--db", default=DB_DEFAULT, help="本地 SQLite 路径")
    parser.add_argument("--force", action="store_true", help="非交易日强制运行")
    parser.add_argument("--boot-force", action="store_true", help="忽略当天已执行标记强制重跑")
...
def main() -> int:
    ...
    if args.reupload:
        ...
        df = load_candidates(args.db, scan_date=args.reupload, table=TABLE)
        if df.empty:
    ...
    if not args.force and cfg.get("check_trading_day", True) and not is_trading_day(today):
        print(f"[job] {today} 非交易日，跳过")
        return 0

    _ensure_heavy_modules()
    _prevent_sleep_during_scan()
    scan_date = today.isoformat()
    generated_at = time.strftime("%Y-%m-%d %H:%M:%S")
    print(f"[job] 开始超跌反弹扫描：{scan_date}，共 {args.total} 只，{args.batch} 只/批", flush=True)
    ...
    try:
        saved = save_candidates(df, args.db, scan_date, table=TABLE)
        print(f"[job] 本地 SQLite 写入 {saved} 行 → {args.db}（表 {TABLE}）")
    ...
    ok = upload(args.server, args.token, payload)
    if not ok:
        print(f"[job] 上报失败：数据已存本地 SQLite，网络恢复后补传: ./run_once_ob.sh --reupload {scan_date}", file=sys.stderr)
    return 0 if ok else 1
