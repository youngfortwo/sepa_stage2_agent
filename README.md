...
| 当天已成功执行过            | 自动跳过（不重复扫描）    |
| 非交易日                | 自动跳过（新浪交易日历校验） |
| 手动运行 `--boot-force` | 强制重跑，忽略当天已执行标记 |

即：**每天保证成功执行一次**——开机先跑，跑过 18 点不再跑；18 点前没跑过则 18 点跑。重启开机自动补跑当天任务。

**强制重跑**（运行时传参，每次运行每次输入）：

```bash
./run_once.sh --boot-force    # 忽略"当天已执行"标记，强制重新扫描并更新标记
```

与 `--force` 的区别：`--force` 是试跑（不写标记，不影响 18:00 定时任务）；`--boot-force` 是正式重跑（完成后更新标记，当天后续触发不再跑）。两个参数都传时以 `--boot-force` 为准。
...
| `./run_once.sh --boot-force`          | 强制重跑（忽略当天已执行，完成后更新标记）  |
